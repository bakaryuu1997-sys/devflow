...
def signed_import_package(db_path: Path) -> dict:
    ...
    return {
        "version": "9.1",
        "status": "Ready for Signed Payload Import" if package["status"] == "Package Ready" else "Blocked",
...
def verify_signed_payload(db_path: Path, payload_hash: str, signature_hash: str) -> dict:
    ...
    with sqlite3.connect(db_path) as con:
        con.execute(
            """INSERT INTO signed_payload_verification_records (payload_hash, manifest_hash, bundle_hash, signature_algorithm, signer_name, signature_reference, signature_hash, verification_status, notes, content, created_at) VALUES (?, ?, ?, 'external-signature-hash', 'CLI Operator', 'CLI import', ?, ?, '', ?, CURRENT_TIMESTAMP)""",
            (
                payload_hash,
                ...
                signature_hash,
                status,
                render_signed_record(status, payload_hash),
            ),
        )
        con.commit()
...
def timestamp_token_package(db_path: Path) -> dict:
    ...
    return {
        "version": "9.1",
        "status": "Ready for Token Attachment" if package["payload_hash"] else "Blocked",
        "payload_hash": package["payload_hash"],
        "manifest_hash": package["manifest_hash"],
        "bundle_hash": package["bundle_hash"],
    }
...
def render_token_package(data: dict) -> str:
    return (
        f"# v9.1 Timestamp Token Evidence Package\n\nStatus: {data['status']}\nPayload hash: `{data['payload_hash']}`\n"
    )
...
def ensure_tables(db_path: Path) -> None:
    with sqlite3.connect(db_path) as con:
        con.execute(
            """CREATE TABLE IF NOT EXISTS signed_payload_verification_records (id INTEGER PRIMARY KEY, payload_hash TEXT NOT NULL, manifest_hash TEXT DEFAULT '', bundle_hash TEXT DEFAULT '', signature_algorithm TEXT DEFAULT '', signer_name TEXT DEFAULT '', signature_reference TEXT DEFAULT '', signature_hash TEXT DEFAULT '', verification_status TEXT DEFAULT '', notes TEXT DEFAULT '', content TEXT DEFAULT '', created_at TEXT DEFAULT CURRENT_TIMESTAMP)"""
        )
        con.execute(
