...
def main() -> int:
    ...
    verified = bool(frozen and frozen.get("manifest_hash") == current["manifest_hash"] and frozen.get("bundle_hash") == current["bundle_hash"])
    status = "Verified" if verified else "No Frozen Manifest" if not frozen else "Changed Since Freeze"
    lines = ["# v8.9 Export Bundle Integrity Check", "", f"Status: {status}", f"Verified: {verified}", "", "## Hash comparison", f"- Current manifest: `{current['manifest_hash']}`", f"- Frozen manifest: `{frozen.get('manifest_hash', '') if frozen else 'none'}`", f"- Current bundle: `{current['bundle_hash']}`", f"- Frozen bundle: `{frozen.get('bundle_hash', '') if frozen else 'none'}`"]
    out_path.write_text("\n".join(lines).strip() + "\n", encoding="utf-8")
