...
class DevFlowWatcher:
    ...
    def handle_file_change(self, filepath, event_type):
        ...
        # 3. Environment Variables
            ...
            try:
                ...
                payload = json.dumps({"content": content, "required_keys": ["DATABASE_URL", "JWT_SECRET_KEY"]}).encode(
                    "utf-8"
                )
                headers = {"Content-Type": "application/json"}
    ...
    def run(self):
        ...
        try:
            ...
            for changes in watch(self.watch_dir):
                for change_type, filepath in changes:
                    ...
                    event_type = status_map.get(
                        change_type.value if hasattr(change_type, "value") else change_type, "modified"
                    )
            ...
            self.log(
                "Watchfiles library not available. Falling back to native directory polling (CPU-friendly 1.5s interval)."
            )
            mtimes = {}
            while True:
                current_mtimes = {}
                for root, dirs, files in os.walk(self.watch_dir):
                    ...
                    dirs[:] = [
                        d
                        for d in dirs
                        if not d.startswith(".") and d not in ["node_modules", "__pycache__", ".venv", ".pytest_cache"]
                    ]
                    for f in files:
...
def main():
    parser = argparse.ArgumentParser(description="DevFlow Guard auto-sync directory watcher.")
