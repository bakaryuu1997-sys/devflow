...
def _create_project(headers):
    project = client.post(
        "/api/projects", json={"name": "Next Release Planning", "description": "v7.4"}, headers=headers
    ).json()
    return project
...
def test_v74_learning_item_accepts_owner_due_date_and_updates_planning():
    ...
    updated = client.patch(
        f"/api/release-learning-items/{item['id']}/planning",
        json={
...
def test_v74_next_release_readiness_flags_unplanned_and_ready_states():
    ...
    client.patch(
        f"/api/release-learning-items/{first['id']}/planning",
        json={
            "owner": "PM",
            "due_date": "2099-02-02",
            "status": "Prevented",
