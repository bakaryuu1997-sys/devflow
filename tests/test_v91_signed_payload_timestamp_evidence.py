...
def test_v91_signed_payload_import_and_timestamp_token_endpoints():
    ...
    created = client.post(
        "/api/release-governance/signed-payload-verifications",
        json={
...
def test_v91_timestamp_token_attachment_and_integrity_endpoint():
    ...
    client.post(
        ...
        json={"timestamp_authority": "Test TSA", "request_reference": "REQ-91", "response_token_hash": "c" * 64},
    )
    ...
    attached = client.post(
        "/api/release-governance/timestamp-token-evidence-attachments",
        json={
            "payload_hash": token_package["payload_hash"],
            "token_hash": "d" * 64,
            "timestamp_authority": "Test TSA",
...
def test_v91_cli_exports_and_verification(tmp_path):
    ...
    signed = subprocess.run(
        [sys.executable, "scripts/export_signed_payload_import_package.py", str(db_path), str(signed_out)],
        text=True,
        capture_output=True,
        check=False,
    )
    token = subprocess.run(
        [sys.executable, "scripts/export_timestamp_token_evidence_package.py", str(db_path), str(token_out)],
        text=True,
        capture_output=True,
        check=False,
    )
...
def create_v91_database(path: Path) -> None:
    ...
    try:
        ...
        con.execute(
            "CREATE TABLE external_timestamp_handoff_records (id INTEGER PRIMARY KEY, payload_hash TEXT DEFAULT '', manifest_hash TEXT DEFAULT '', bundle_hash TEXT DEFAULT '', timestamp_authority TEXT DEFAULT '', request_reference TEXT DEFAULT '', response_token_hash TEXT DEFAULT '', status TEXT DEFAULT '', notes TEXT DEFAULT '', content TEXT DEFAULT '', created_at TEXT DEFAULT '')"
        )
        con.execute(
