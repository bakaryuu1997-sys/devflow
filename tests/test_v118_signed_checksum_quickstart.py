...
def test_v118_operator_package_docs_ui_and_cli(tmp_path):
    _prepare_profile()
    package = client.get(
        "/api/release-governance/v11-8-operator-checksum-quickstart-package?profile_id=core-risk"
    ).json()
    ...
    assert "governance_v118_ui.js" in Path("static/governance_bundle.js").read_text(encoding="utf-8")
    out = tmp_path / "v11_8.md"
    result = subprocess.run(
        [sys.executable, "scripts/export_v11_8_checksum_quickstart_package.py", str(out)],
        text=True,
        capture_output=True,
        check=False,
    )
