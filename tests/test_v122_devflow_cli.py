...
def test_cli_help_menu():
    ...
    result = subprocess.run(
        [sys.executable, "scripts/devflow_cli.py", "--help"],
...
def test_cli_subcommands_help():
    ...
    result = subprocess.run(
        [sys.executable, "scripts/devflow_cli.py", "scan", "--help"],
        capture_output=True, text=True
    )
    assert result.returncode == 0
    ...
    result = subprocess.run(
        [sys.executable, "scripts/devflow_cli.py", "status", "--help"],
        capture_output=True, text=True
    )
    assert result.returncode == 0
    ...
    result = subprocess.run(
        [sys.executable, "scripts/devflow_cli.py", "install-hook", "--help"],
        capture_output=True, text=True
    )
    assert result.returncode == 0
