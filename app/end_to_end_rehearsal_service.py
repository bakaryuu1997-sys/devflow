...
def list_governance_rehearsals(db: Session) -> dict:
    ...
    data = {
        "version": "9.9",
        "mode": "governance-rehearsal-records",
        "count": len(rows),
...
def _row(row: GovernanceRehearsalRecord) -> dict:
    return {
        ...
        "blockers": row.blockers.splitlines() if row.blockers else [],
...
def _markdown(data: dict) -> str:
    lines = [
        "# v9.9 End-to-End Production Release Governance Rehearsal",
        "",
        f"Status: {data['status']}",
        f"Readiness score: {data['readiness_score']}",
        "",
        "## Sections",
    ]
