...
def v10_1_operator_quickstart_package() -> dict:
    ...
    data["content"] = "# v10.1 Operator Quickstart Package\n\n" + "\n\n".join(
        [walkthrough["content"], demo["content"], deployment["content"]]
    )
    return data
...
def _walkthrough_steps() -> list[dict]:
    return [
        ...
        {
            "step": 3,
            "name": "Review prevention",
            "action": "Open prevention board, readiness timeline, and plan recommendation.",
        },
        {
            "step": 4,
            "name": "Review governance",
            "action": "Open evidence manifest, verified gate, final bundle, and rehearsal report.",
        },
        {
            "step": 5,
            "name": "Prepare handoff",
            "action": "Export operator handoff, runbook, final evidence bundle, and v10 package.",
        },
    ]
...
def _demo_sections() -> list[dict]:
    return [
        ...
        {
            "title": "Prevention",
            "talk_track": "Recurring risks become prevention items with owners, due dates, and readiness views.",
        },
        {
            "title": "Governance",
            "talk_track": "Approval evidence is frozen, hashed, verified, and exported as an operator package.",
        },
        {
            "title": "Safety",
            "talk_track": "Production migration is copy-tested first and requires explicit human approval.",
        },
    ]
...
def _deployment_modes() -> list[dict]:
    return [
        {"mode": "local-dev", "command": "uvicorn app.main:app --reload", "use_case": "daily development and demos"},
        {
            ...
            "use_case": "single-machine operator review",
        },
        {
            "mode": "packaged-handoff",
            "command": "python scripts/export_v10_1_operator_quickstart.py V10_1_OPERATOR_QUICKSTART.md",
            "use_case": "handoff document export",
        },
    ]
