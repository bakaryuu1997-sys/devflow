...
def risk_rules(requirement: dict, work_items: list[dict]) -> list[dict]:
    ...
    if requirement["priority"] in CRITICAL_PRIORITIES and not tests:
        risks.append(
            _risk("critical_requirement_without_test", "Critical requirement has no test coverage.", "High", True)
        )
    if not tasks:
        risks.append(_risk("requirement_without_task", "Requirement has no implementation task.", "Medium", False))
    open_blocking_bugs = [
        bug for bug in bugs if bug["status"] in OPEN_STATUSES and bug["severity"] in BLOCKING_BUG_SEVERITIES
    ]
