...
def current_evidence_manifest(db: Session) -> dict:
    ...
    items = (
        [_artifact_item(row) for row in artifacts]
        + [_approval_item(row) for row in approvals]
        + [_verifier_item(row) for row in verifier]
    )
...
def list_evidence_manifests(db: Session) -> dict:
    ...
    data = {
        "version": "8.9",
        "mode": "evidence-manifests",
        "count": len(rows),
        "manifests": [_record_row(row) for row in rows],
    }
...
def export_bundle_integrity_check(db: Session) -> dict:
    ...
    if not latest:
        ...
        reason = (
            "Current evidence matches latest frozen manifest."
            if match
            else "Evidence changed after the latest frozen manifest."
        )
    data = {
        "version": "8.9",
...
def _manifest_list_markdown(data: dict) -> str:
    ...
    lines.extend(
        f"- #{row['id']} {row['status']} items={row['item_count']} manifest={row['manifest_hash']}"
...
def _integrity_markdown(data: dict) -> str:
    lines = [
        ...
        "",
        "## Hash comparison",
        f"- Current manifest: `{data['current_manifest_hash']}`",
        f"- Frozen manifest: `{data['frozen_manifest_hash'] or 'none'}`",
        f"- Current bundle: `{data['current_bundle_hash']}`",
        f"- Frozen bundle: `{data['frozen_bundle_hash'] or 'none'}`",
    ]
    return "\n".join(lines).strip() + "\n"
