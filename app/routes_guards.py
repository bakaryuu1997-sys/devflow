...
async def upload_sql(project_id: int, file: UploadFile = File(...), db: Session = Depends(get_db), _user: User = Depends(require_write)):
...
async def upload_logs(project_id: int, file: UploadFile = File(...), db: Session = Depends(get_db), _user: User = Depends(require_write)):
    content = (await file.read()).decode("utf-8", errors="ignore")
...
async def upload_tests(project_id: int, file: UploadFile = File(...), db: Session = Depends(get_db), _user: User = Depends(require_write)):
    content = (await file.read()).decode("utf-8", errors="ignore")
...
async def upload_api_diff(project_id: int, before: UploadFile = File(...), after: UploadFile = File(...), db: Session = Depends(get_db), _user: User = Depends(require_write)):
...
def evidence(project_id: int, release_id: int | None = None, db: Session = Depends(get_db)):
    return {"project_id": project_id, "release_id": release_id, "content": build_evidence_markdown(db, project_id, release_id)}
