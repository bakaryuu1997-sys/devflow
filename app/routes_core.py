...
def reset_demo(db: Session = Depends(get_db)):
    ...
    db.add(
        TraceLink(
            ...
            link_type="api",
            target_key="POST /api/auth/login",
            title="Login API",
            status="Changed",
            module="auth",
        )
    ...
    db.add(
        TraceLink(
            ...
            link_type="commit",
            target_key="abc123",
            title="TASK-LOGIN-API update auth flow",
            status="Merged",
            module="auth",
        )
    )
...
def api_create_requirement(
    project_id: int, payload: RequirementCreate, db: Session = Depends(get_db), _user=Depends(require_write)
):
...
def api_update_requirement(
    requirement_id: int, payload: RequirementUpdate, db: Session = Depends(get_db), _user=Depends(require_write)
):
...
def api_create_work_item(
    project_id: int, payload: WorkItemCreate, db: Session = Depends(get_db), _user=Depends(require_write)
):
...
def api_update_work_item(
    item_id: int, payload: WorkItemUpdate, db: Session = Depends(get_db), _user=Depends(require_write)
):
...
def api_create_release(
    project_id: int, payload: ReleaseCreate, db: Session = Depends(get_db), _user=Depends(require_release)
):
...
def verify_ledger(db: Session = Depends(get_db)):
    ...
    return {"status": "verified", "count": len(logs)}
