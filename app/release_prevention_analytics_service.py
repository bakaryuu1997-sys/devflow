...
def owner_workload_balance(db: Session, project_id: int, today: date | None = None) -> dict:
    ...
    overloaded = [
        row for row in owner_rows if row["owner"] != "Unassigned" and row["workload_score"] >= max(8, avg_open * 4)
    ]
...
def _owner_status(
    owner: str, open_items: list[dict], overdue: list[dict], due_soon: list[dict], escalated: list[dict], score: int
) -> str:
...
def _burndown_hints(
    open_rows: list[dict], overdue: list[dict], due_soon: list[dict], projection: list[dict]
) -> list[str]:
    hints = []
