...
REQUIRED_SCHEMA = {
    ...
    "external_verifier_profiles": ["name", "adapter", "policy_preset", "key_reference", "status", "notes", "created_at"],
    "final_signed_evidence_bundles": ["manifest_hash", "bundle_hash", "verifier_evidence_id", "profile_name", "status", "content", "created_at"],
...
def local_database_migration_check(db: Session) -> dict:
    ...
    optional_rows = [_optional_status(inspector, tables, table, cols) for table, cols in OPTIONAL_HISTORY_FIELDS.items()]
...
def _table_status(inspector, tables: set[str], table: str, required_columns: list[str]) -> dict:
    if table not in tables:
        return {"table": table, "state": "MISSING_TABLE", "required_columns": required_columns, "missing_columns": required_columns}
    existing = {col["name"] for col in inspector.get_columns(table)}
    missing = [name for name in required_columns if name not in existing]
    return {"table": table, "state": "PASS" if not missing else "MISSING_COLUMNS", "required_columns": required_columns, "missing_columns": missing}
...
def _optional_status(inspector, tables: set[str], table: str, columns: list[str]) -> dict:
    ...
    return {"table": table, "state": "PASS" if not missing else "OPTIONAL_MISSING", "optional_columns": columns, "missing_columns": missing}
...
def _steps(missing_tables: list[str], missing_columns: list[dict]) -> list[str]:
    ...
    for row in missing_columns:
        steps.extend(f"Add column {col} to {row['table']} with nullable/default-safe settings." for col in row["missing_columns"])
...
def _recommended_order(check: dict) -> list[str]:
    return ["1. Backup database", "2. Apply additive schema changes", "3. Start app", "4. Run smoke tests", "5. Export governance readiness report"] + check["upgrade_steps"]


def _markdown(data: dict) -> str:
    lines = ["# v8.1 Local Database Migration Check", "", f"Status: {data['status']}", f"Safe to upgrade: {data['safe_to_upgrade']}", "", "## Required schema"]
    lines.extend(f"- {row['table']}: {row['state']} missing={', '.join(row['missing_columns']) or 'none'}" for row in data["required_schema"])
...
def _safety_markdown(data: dict) -> str:
    lines = ["# v8.1 Upgrade Safety Report", "", f"Status: {data['status']} ({data['risk_score']}/100)", f"Safe to upgrade: {data['safe_to_upgrade']}", "", "## Must fix"]
