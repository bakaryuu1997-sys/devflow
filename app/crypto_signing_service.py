...
def list_timestamp_handoffs(db: Session) -> dict:
    ...
    data = {
        "version": "9.0",
        "mode": "external-timestamp-handoffs",
        "count": len(rows),
        "records": [_record_row(row) for row in rows],
    }
...
def timestamp_handoff_integrity_check(db: Session) -> dict:
    ...
    match = bool(
        latest and latest.payload_hash == package["payload_hash"] and latest.bundle_hash == package["bundle_hash"]
    )
    data = {
        "version": "9.0",
...
def _operator_steps(blockers: list[str]) -> list[str]:
    if blockers:
        return [
            ...
            "Freeze evidence again after any evidence changes.",
            "Re-run bundle integrity check.",
        ]
    return [
        "Export the canonical payload.",
        "Submit payload_hash to the external signing or timestamp authority.",
        "Store the returned token hash/reference in DevFlow Guard.",
    ]
...
def _handoff_steps(readiness: dict) -> list[str]:
    ...
    return [
        "Export this handoff package.",
        "Send only the payload hash to the external timestamp service if policy allows.",
        "Record request reference and returned token hash.",
        "Run timestamp handoff integrity check.",
    ]
...
def _handoff_markdown(data: dict) -> str:
    ...
    lines.extend(
        ["", "## Canonical payload", "```json", json.dumps(data["canonical_payload"], indent=2, sort_keys=True), "```"]
    )
    return "\n".join(lines).strip() + "\n"
...
def _handoff_list_markdown(data: dict) -> str:
    ...
    lines.extend(
        f"- #{row['id']} {row['status']} authority={row['timestamp_authority']} payload={row['payload_hash']}"
...
def _integrity_markdown(data: dict) -> str:
    lines = [
        "# v9.0 Timestamp Handoff Integrity Check",
        "",
        f"Status: {data['status']}",
        f"Verified: {data['verified']}",
        f"Current payload: `{data['current_payload_hash']}`",
        f"Handoff payload: `{data['handoff_payload_hash'] or 'none'}`",
    ]
    return "\n".join(lines).strip() + "\n"
